...
const DisclaimerModal: React.FC<DisclaimerModal> = ({ onConfirm, onDismiss }) => {
  ...
  const modalContent = useMemo(() => {
    if (step === 'disclaimer') {
      ...
      return (
        <div>
          <p>The only Uniswap pool that is compatible with reB∆SE is reB∆SE/USDC</p>
        </div>
      )
    }
  }, [step])
